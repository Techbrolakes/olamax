...
export function TvRail({ title, seeAllHref, shows, priority }: Props) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="scrollbar-thin flex gap-3 overflow-x-auto scroll-smooth px-4 pb-4 md:px-8">
        {shows.map((show, i) => (
          <div key={show.id} className="w-[128px] flex-none md:w-[150px] lg:w-[170px]">
            <TvCard show={show} priority={priority && i < 6} />
          </div>
        ))}
      </div>
    </section>
