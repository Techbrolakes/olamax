...
type Category = "trending" | "popular" | "top-rated" | "upcoming";

type Props = {
  category: Category;
  initialData: Paginated<Movie>;
};

const SKELETON_COUNT = 16;

async function fetchPage(category: Category, page: number): Promise<Paginated<Movie>> {
  const res = await fetch(`/api/tmdb/movies/${category}?page=${page}`);
  if (!res.ok) throw new Error("Failed to load movies");
  return res.json();
}

export function MoviesInfiniteList({ category, initialData }: Props) {
      ...
      <div className="grid grid-cols-3 gap-x-3 gap-y-6 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8">
        ...
        {isFetchingNextPage
          ? Array.from({ length: SKELETON_COUNT }).map((_, i) => (
              <MovieSkeleton key={`sk-${i}`} delay={i * 60} />
            ))
          : null}
      </div>
...
function MovieSkeleton({ delay }: { delay: number }) {
  return (
